...
func UpdateCensusTopic(ctx context.Context, censusTopicID string, topicClient topicCli.Clienter) func() *cache.Topic {
	return func() *cache.Topic {
		// TODO can put two requests in Go routines

		// get census topic from dp-topic-api
		censusTopic, err := topicClient.GetTopicPublic(ctx, topicCli.Headers{}, censusTopicID)
		if err != nil {
			logData := log.Data{
		...
		censusSubtopics, err := topicClient.GetSubtopicsPublic(ctx, topicCli.Headers{}, censusTopicID)
		if err != nil {
			logData := log.Data{
				"req_headers": topicCli.Headers{},
			}
		...
		censusTopicCache := setTopicCachePublic(ctx, *censusTopic, *censusSubtopics.PublicItems)
...
func setTopicCachePublic(ctx context.Context, censusTopic models.Topic, subtopics []models.Topic) *cache.Topic {
	censusTopicCache := &cache.Topic{
		ID:              censusTopic.ID,
		LocaliseKeyName: censusTopic.Title,
	}

	subtopicCache := cache.NewSubTopicsMap()
	for i := range subtopics {
		subtopicCache.AppendSubtopicItems(&subtopics[i])
	}
